...
|---|---|---|---|
| Quản lý phiên bản Git & Release Management | Khởi tạo cấu trúc phân nhánh (`develop`, `Quan`, `VanDien`, `Quang`), thiết lập workflow PR và tiêu chuẩn kiểm thử tự động (gating tests); trực tiếp điều phối, review code và giải quyết xung đột mã nguồn (merge conflicts) khi tích hợp các nhánh độc lập: PR #1 (RAG base), PR #2 (`6bf57fc`, tích hợp Retrieval), PR #3 (`66de668`, audit & +10 bonus suite), PR #5 (`b3b8eff`, báo cáo & TEAMMATES); bảo đảm nhánh `develop` luôn ổn định và passing 100% tests. | PR #1, PR #2, PR #3, PR #5, commits `6bf57fc`, `66de668`, `b3b8eff` | Done |
| Lập trình Script Thu thập & Chuẩn hóa Dữ liệu (Task 1–3) | Lập trình trọn bộ 3 script pipeline dữ liệu tự động: (1) `src/task1_collect_legal_docs.py` tự động nạp văn bản quy chế từ website HCMUS vào landing; (2) `src/task2_crawl_news.py` cào tin tức thông báo công tác sinh viên bằng custom TextExtractor / HTML-to-Markdown parser; (3) `src/task3_convert_markdown.py` chuyển đổi PDF/DOCX sang Markdown có YAML metadata chuẩn, xử lý bóc tách text layer từ `STSV2025_ONLINE.pdf` thay thế PDF scan mộc đỏ mờ, đảm bảo dữ liệu sạch 100% cho ChromaDB. | `src/task1_collect_legal_docs.py`, `src/task2_crawl_news.py`, `src/task3_convert_markdown.py`, `data/`, commits `cac0bef`, `fa929e3`, PR #1 | Done |
| Audit, Kiểm định Chất lượng & Tinh chỉnh Code Cuối kỳ (Tasks 4–10) | Rà soát toàn diện chất lượng code và hợp đồng dữ liệu từ Task 4 đến Task 10 trước khi nộp sản phẩm: bổ sung chunk deduplication & hashing (Task 4); xử lý tương thích chữ ký mock embedding `input_type` và chuẩn hóa khoảng cách cosine $[0, 1]$ (Task 5); chuẩn hóa regex tokenizer tiếng Việt cho BM25 (Task 6); bảo toàn deduplication trong RRF $k=60$ (Task 7); nâng cấp PageIndex cấu trúc tiêu đề `#`, `##` (Task 8); hiệu chỉnh routing điểm tin cậy và ngưỡng fallback `SCORE_THRESHOLD = 0.60` (Task 9); viết hàm chuẩn hóa trích dẫn `_normalize_citations` và exponential backoff retry khi gặp lỗi Gemini 429 quota (Task 10). Nâng điểm benchmark trung bình từ 0.736 lên 0.889. | `src/task4_chunking_indexing.py` đến `src/task10_generation.py`, commits `75634b3`, `f69d2ea`, PR #3 | Done |
...
| Kiểm thử tự động & Quản lý Chất lượng (Test Suite) | Thiết kế bộ test hợp đồng (`test_contracts.py`), kiểm thử luồng tích hợp (`test_acceptance.py`), và bổ sung trọn bộ 6 unit tests chuyên biệt cho các tính năng bonus (`test_bonus_features.py`), bảo đảm 26/26 tests vượt qua tự động 100% trong 5.89s; kiểm soát chất lượng code trước khi merge vào nhánh chính `develop`. | `tests/test_contracts.py`, `tests/test_acceptance.py`, `tests/test_bonus_features.py`, commits `566c05d`, `66de668` (PR #3) | Done |

## Quyết định kỹ thuật quan trọng

1. **Quyết định:** Sử dụng tài liệu Sổ tay sinh viên số hóa 2025 (`STSV2025_ONLINE.pdf`) có text layer sạch 100% làm nguồn trích xuất dữ liệu gốc cho script `src/task3_convert_markdown.py`, kết hợp chuẩn hóa Metadata YAML phân cấp (`doc_id`, `title`, `doc_type`, `effective_date`) thay vì chạy OCR trên các bản scan quyết định gốc (QĐ 1028, QĐ 1175).  
...
- Test hoặc query tôi đã dùng:
  - Kiểm thử script pipeline: Chạy tự động `python src/task1_collect_legal_docs.py`, `python src/task2_crawl_news.py`, `python src/task3_convert_markdown.py` kiểm tra sinh thành công toàn bộ tài liệu Markdown có YAML metadata hợp lệ.
...
- Kết quả trước/sau nếu có:
  - **Trước khi Audit & Enhance:**
    ...
    - Thiếu cơ chế khử trùng lặp và chuẩn hóa khoảng cách cosine $[0, 1]$ khiến điểm similarity bị âm hoặc vượt ngưỡng.
  - **Sau khi Audit, Enhance & Thêm Bonus Suite:**
    ...
    - Toàn bộ nhánh và PR hợp nhất mượt mà vào `develop` không xảy ra conflict hay phá vỡ contracts.
